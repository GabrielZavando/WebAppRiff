 ...
 * page (the first is the HeroBanner, `section.relative:not(.z-10)` — the
 * full-bleed overlay change removed `bg-secondary` from the hero). We target by
...
test.describe('PanelHome (home about/trust panel)', () => {
  ...
  test('the <h1> "Innovación que Fluye" stays visible above the panel on common viewports (4.8)', async ({
    ...
    for (const vp of viewports) {
      ...
      const h1 = page.locator('section.relative:not(.z-10) h1').first();
