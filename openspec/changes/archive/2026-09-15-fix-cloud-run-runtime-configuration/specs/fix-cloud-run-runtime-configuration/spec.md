# fix-cloud-run-runtime-configuration Specification — DELTA

> Este delta configura el runtime de Cloud Run para el backend NestJS en staging
> (service account, env vars Firebase/CORS, `NODE_ENV`, acceso público, secrets de
> Secret Manager), parametriza rutas con repo vars y mantiene la lane de producción
> bloqueada. Añade la capacidad `fix-cloud-run-runtime-configuration`.
