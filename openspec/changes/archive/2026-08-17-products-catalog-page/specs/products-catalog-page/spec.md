...
Define the public catalog page at `/productos` of the Riff Astro SSG site:
build-time data sourcing (all products + categories + subcategories baked into
the static page), a client-side runtime that reads URL query params and
applies filter / pagination / view-mode without a full navigation
(progressive enhancement over the static, unfiltered render), the
category→subcategory sidebar filter pattern, the mobile-first card layout,
and a no-JavaScript fallback (the static HTML shows the full catalog;
filtering requires JS).
