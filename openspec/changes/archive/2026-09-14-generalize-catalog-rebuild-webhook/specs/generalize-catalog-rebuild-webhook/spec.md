# generalize-catalog-rebuild-webhook Specification — DELTA (generalize-catalog-rebuild-webhook)

> Este delta generaliza el notifier de regeneración del sitio Astro: de un
> webhook de categorías (`CATEGORIES_WEBHOOK_URL`, sin auth) a un notifier de
> catálogo unificado (`CATALOG_REBUILD_*` con Bearer + timeout) que cubre
> categorías, subcategorías y productos. Añade una nueva capacidad
> `generalize-catalog-rebuild-webhook`.
