# use-adc-firebase-admin Specification — DELTA (use-adc-firebase-admin)

> Este delta cambia cómo `FirebaseModule` (backend) autentica ante Firebase Admin
> en runtime: de un service account explícito (`cert()` + 3 env vars) a
> **Application Default Credentials** (`applicationDefault()`), que en Cloud Run
> resuelve automáticamente la service account de runtime del servicio. Añade una
> nueva capacidad `use-adc-firebase-admin` en `openspec/specs/`.
