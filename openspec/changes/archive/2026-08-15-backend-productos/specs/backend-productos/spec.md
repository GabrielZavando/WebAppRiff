...
## Purpose
Implementa el módulo backend (NestJS BFF) de `productos`: endpoints de lectura pública (consumidos por el sitio Astro SSG, solo `publicado:true` para anónimos) y CRUD autenticado con roles (editor=edición, admin/superadmin=crear/borrar), aplicando las reglas de dominio documentadas (unicidad global de SKU/slug, categoría por defecto `sin-categoria`, consistencia categoría/subcategoría, galería ≤10, validación de metadata de ficha técnica PDF). Galería/ficha son metadata validada; el upload binario a Storage queda fuera de alcance.
