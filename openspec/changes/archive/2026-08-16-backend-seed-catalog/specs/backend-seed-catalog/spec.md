# backend-seed-catalog Specification — DELTA (backend-seed-catalog)

> This delta adds a new capability spec at `openspec/specs/backend-seed-catalog/spec.md`.
> The seed CLI populates the `categorias` and `subcategorias` Firestore collections idempotently from `seed-categorias-subcategorias.json`, using deterministic document ids (slug for categories, `{categoriaSlug}--{subcategoriaSlug}` for subcategorias), creating `sin-categoria` with `esDefault: true`, skipping existing entries, and deriving the slug from `nombre` when not provided.
