# firestore-migration Specification

## Purpose
Script y proceso operativo para copiar las colecciones del catálogo (`categorias`,
`subcategorias`, `productos`, `cotizaciones`) entre proyectos Firebase preservando los
IDs de documento, de forma idempotente, excluyendo `usuarios` y con copia condicional
de Storage y modo `--dry-run`. Implementado con `firebase-admin` (Admin SDK dual
SOURCE/DEST) en `apps/backend/src/cli/migrate/`.
