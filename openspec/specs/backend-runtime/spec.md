...
## Purpose
Define el esqueleto runnable del backend NestJS (apps/backend) del catálogo Riff: bootstrap de la aplicación, resolución del puerto de escucha (incluido fallback a 3000 para valores inválidos), endpoint público de health check `GET /health`, configuración TypeScript estricta, umbrales SOLID de ESLint, cobertura mínima de tests (90%), imagen Docker multi-stage y variables de entorno documentadas. Es la fundación sobre la que se montan los módulos de dominio y la infraestructura (Firebase, Auth, CORS, Throttler) en changes posteriores.
## Requirements
