# backend-firebase Specification

## Purpose
Define la infraestructura Firebase del backend NestJS (BFF) Riff: inicialización de la aplicación `firebase-admin` desde un service account leído de variables de entorno, provisión global de la instancia de Firestore vía un token de inyección dedicado, helpers puros y testeables para construir el service account y normalizar la private key, y fallo rápido (fail-fast) ante credenciales faltantes. Es la base sobre la que se montan los módulos de dominio (categorías, subcategorías, productos) que inyectarán Firestore sin importar `firebase-admin` directamente (DIP — ver `docs/backend-standards.md`).
